...
import { Bell, Sun, Moon, LogOut, User, Settings, ChevronDown, Menu, X, Leaf, Languages } from 'lucide-react';
...
export default function Navbar({ onMenuToggle, sidebarOpen }) {
  ...
  useEffect(() => {
    const handler = (e) => { if (dropRef.current && !dropRef.current.contains(e.target)) { setDropOpen(false); setNotifOpen(false); } };
  ...
  const NOTIFS = [
    { id:1, msg: lang==='ta' ? 'உங்கள் நில கோரிக்கை #1234 ஆய்வில் உள்ளது' : 'Your land request #1234 is under review', time:'2h ago', unread:true },
    { id:2, msg: lang==='ta' ? 'இழப்பீடு கணிப்பு முடிந்தது' : 'Compensation prediction completed', time:'1d ago', unread:true },
    { id:3, msg: lang==='ta' ? 'ஞாயிறு அதிகாலை 2 மணிக்கு பராமரிப்பு' : 'System maintenance Sunday 2AM', time:'2d ago', unread:false },
  ];
  ...
  return (
    <header className="sticky top-0 z-40 h-16 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-200/60 dark:border-slate-700/40 flex items-center px-4 gap-4">
      ...
      <Link to="/" className="flex items-center gap-2 font-display font-bold text-lg text-slate-800 dark:text-white mr-auto lg:hidden">
        <div className="w-8 h-8 bg-primary-600 rounded-lg flex items-center justify-center"><Leaf size={16} className="text-white"/></div>
        AgriComp
      </Link>

      <div className="flex items-center gap-1.5 ml-auto" ref={dropRef}>
        {/* Language toggle */}
        <motion.button whileTap={{scale:0.9}} onClick={toggleLang}
          className="flex items-center gap-1.5 px-3 py-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors text-sm font-bold"
          title="Toggle Tamil/English">
          <Languages size={16} className="text-primary-600 dark:text-primary-400"/>
          <span className={`text-xs font-extrabold px-1.5 py-0.5 rounded-lg ${lang==='ta'?'bg-primary-100 text-primary-700 dark:bg-primary-900/40 dark:text-primary-400':'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300'}`}>
            {lang==='ta' ? 'தமிழ்' : 'EN'}
          </span>
          <span className={`text-xs font-bold ${lang==='ta'?'text-slate-400':'text-primary-600 dark:text-primary-400'}`}>
            {lang==='ta' ? '→ EN' : '→ தமிழ்'}
          </span>
        </motion.button>

        {/* Dark mode */}
        <motion.button whileTap={{scale:0.9}} onClick={toggleTheme} className="btn-ghost p-2" title={t('darkMode')}>
        ...
        <div className="relative">
          <motion.button whileTap={{scale:0.9}} onClick={()=>{setNotifOpen(!notifOpen);setDropOpen(false);}} className="btn-ghost p-2 relative">
            <Bell size={18}/>
            ...
            {notifOpen && (
              <motion.div initial={{opacity:0,y:8,scale:0.95}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:8,scale:0.95}} transition={{duration:0.15}}
                ...
                <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                  <span className="font-semibold text-sm text-slate-800 dark:text-white">{t('notifications')}</span>
        ...
        {/* Profile */}
        <div className="relative">
          <motion.button whileTap={{scale:0.97}} onClick={()=>{setDropOpen(!dropOpen);setNotifOpen(false);}}
            ...
            <div className="hidden sm:block text-left">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 capitalize">{user?.role==='admin'?t('adminRole'):t('farmerRole')}</p>
            </div>
            ...
            {dropOpen && (
              <motion.div initial={{opacity:0,y:8,scale:0.95}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:8,scale:0.95}} transition={{duration:0.15}}
                ...
                <Link to={user?.role==='admin'?'/admin/reports':'/farmer/reports'} onClick={()=>setDropOpen(false)}
                  className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <Settings size={15}/> {t('reports')}
                </Link>
                <div className="mt-1 pt-1 border-t border-slate-100 dark:border-slate-700">
                  <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
